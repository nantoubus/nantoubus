...
namespace Nantou_bus
{
    public class StopDetailsView : ContentPage
    {
        ...
        public StopDetailsView(double latitude, double longitude, String stopID, String stopName) : this()
        {
            Title = stopName;
            ToolbarItem tbi = new ToolbarItem();
            tbi.Text = "顯示行經路線";
            tbi.Clicked += (object sender, EventArgs e) => { this.Navigation.PushAsync(new StopPassingRouteView(stopName,stopID)); };
            this.ToolbarItems.Add(tbi);
            PageLayout.Children.Add(new GoogleStreetView(latitude, longitude));
            PopulateArriveRoute(stopID, stopName);
